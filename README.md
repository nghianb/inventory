...
- Giải quyết ghi kết quả cho mọi Đơn vị hàng còn trong khiếu nại: Bồi hoàn tiền (số tiền > 0, ngày không sau hôm nay), Hàng thay thế hoặc Bị từ chối (CHECK `supplier_claim_units_outcome`). Ngày giải quyết là `supplier_claims.resolved_at`.
- Xem mã ở bảng Đơn vị hàng của khiếu nại (`ContentReveal::revealClaimUnit`): khi Đơn vị hàng còn trong khiếu nại (kể cả Đã giải quyết) và còn Lỗi, ghi Nhật ký xem mã ngữ cảnh Khiếu nại nhà cung cấp cho cả Đơn vị hàng (`slot_id` trống).
- Hàng thay thế: nút Nhập hàng thay thế mở trang tạo Lô nhập với `?khieu-nai={id}`. Lô nhập bình thường liên kết với khiếu nại (`batches.supplier_claim_id`), chỉ cho Khiếu nại Đã giải quyết có kết quả Hàng thay thế, cùng Nhà cung cấp; mọi Dòng nhập Giá vốn 0 và cột `gia_von` của file bị bỏ qua.
